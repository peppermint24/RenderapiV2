...
class DataStore:
    def __init__(self):
        self.ses_tree: Optional[STRtree] = None
        self.ses_geoms: List = []
        self.ses_geoids: List[str] = []
        self.ses_scores: Dict[str, float] = {}
        self.humidity_data: Dict[str, float] = {}
        self.flood_claims: Dict[str, float] = {}
        self.last_reload: Dict[str, str] = {}
...
def load_humidity_data():
    ...
    try:
        ...
        return {"rows_loaded": len(data_store.humidity_data), "path": HUMIDITY_PATH}
    except Exception as e:
...
def load_flood_claims():
    ...
    try:
        ...
        return {"rows_loaded": len(data_store.flood_claims), "path": CLAIMS_PATH}
    except Exception as e:
...
# ====================
# SES Functions (robust)
# ====================

def point_in_polygon_lookup(lat: float, lon: float) -> Tuple[Optional[str], Optional[float]]:
    """Find census tract GEOID for a point using STRtree (robust across Shapely versions)."""
    if not data_store.ses_tree:
        return None, None

    point = Point(lon, lat)

    # 1) Prefer query_bulk to get indices directly (Shapely 2.x)
    idxs: List[int] = []
    try:
        qb = data_store.ses_tree.query_bulk([point])
        # qb is (2, N) array: first row is input geom index (0), second row candidate indices
        idxs = qb[1].tolist()
    except Exception:
        # 2) Fallback to query; attempt to get indices if provided, else scan all
        try:
            candidates = data_store.ses_tree.query(point)
            if isinstance(candidates, np.ndarray) and candidates.dtype.kind in {"i", "u"}:
                idxs = candidates.tolist()
            elif isinstance(candidates, (list, tuple)) and candidates and isinstance(candidates[0], int):
                idxs = list(candidates)
            else:
                # Last resort: check all (slower, but safe)
                idxs = list(range(len(data_store.ses_geoms)))
        except Exception:
            idxs = list(range(len(data_store.ses_geoms)))

    # 3) Exact topological check
    for idx in idxs:
        try:
...
# ====================
# External API Functions (NFHL fixes)
# ====================

async def query_fema_nfhl(lat: float, lon: float) -> Dict[str, Any]:
    """Query FEMA NFHL for flood zone information (point-in-polygon)."""
    url = "https://hazards.fema.gov/gis/nfhl/rest/services/public/NFHL/MapServer/28/query"
    params = {
        "f": "json",
        "returnGeometry": "false",
        # Use intersects to find polygons containing the point
        "spatialRel": "esriSpatialRelIntersects",
        # ArcGIS expects JSON point geometry
...
def calculate_nfhl_score(nfhl_data: Dict[str, Any]) -> float:
    """Convert NFHL zone to 0-1 safety score (higher = safer)."""
    if not nfhl_data:
        return 0.5

    zone = (nfhl_data.get("zone") or "").upper()
    subtype = (nfhl_data.get("zone_subtype") or "").upper()
    depth = nfhl_data.get("depth_ft") or 0
    floodway = bool(nfhl_data.get("floodway"))

    # Floodway: very unsafe
    if floodway:
        return 0.2
    ...
    # Moderate risk (Shaded X or explicit subtype)
    if "SHADED" in subtype:
        return 0.5
        ...
        async with httpx.AsyncClient(timeout=20.0) as client:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    if year_match:
                        return {"year": int(year_match.group()), "source": "perplexity", "confidence": "low"}
    except Exception as e:
...
async def reload_ses():
    ...
    return ReloadResponse(success=True, message="SES data reloaded successfully", details=details)
...
async def reload_humidity():
    ...
    return ReloadResponse(success=True, message="Humidity data reloaded successfully", details=details)
...
async def reload_flood_claims():
    ...
    return ReloadResponse(success=True, message="Flood claims data reloaded successfully", details=details)
...
async def get_ses_score(lat: float = Query(...), lon: float = Query(...)):
    ...
    return {"geoid": geoid, "ses_score": score, "source": "ses:tracts_wkb.parquet + tract_lookup.json"}
...
async def get_build_year(request: BuildYearRequest):
    ...
    stars = (score / 100 * 5) if score is not None else None

    return {
        "year": year,
        "source": result.get("source", "perplexity"),
        "confidence": result.get("confidence", "low"),
        "stars_5": round(stars, 1) if stars is not None else None,
        "score_0_100": score,
    }
...
async def get_flood_score(
    ...
    # Combine only available parts to avoid biasing toward 0.5 by default
    parts = [nfhl_score]
    if claims_proportion is not None:
        parts.append(1 - float(claims_proportion))  # safety = 1 - proportion

    flood_safety = sum(parts) / len(parts)
    ...
    return {
        ...
        "claims": {
            ...
            "claims_safety_0_1": (1 - float(claims_proportion)) if claims_proportion is not None else None,
        },
...
async def get_humidity_score(zip: str = Query(...)):
    ...
    if risk_score is not None:
        stars = round((float(risk_score) / 100) * 5, 1)
        return {"zip": zip5, "humidity_risk_0_100": float(risk_score), "humidity_risk_stars_5": stars}
    else:
        return {"zip": zip5, "humidity_risk_0_100": None, "humidity_risk_stars_5": None}
...
async def get_total_score(
    ...
    if ses_score is not None:
        components["ses"] = {"geoid": geoid, "score_0_100": float(ses_score)}
    ...
    if flood_safety_100 is not None:
        components["flood_safety"] = {
            "score_0_100": float(flood_safety_100),
            "details": {"nfhl": flood_result["nfhl"], "claims": flood_result["claims"]},
        }
    ...
    if zip:
        ...
        if humidity_risk is not None:
            humidity_safety = 100.0 - float(humidity_risk)
            components["humidity_safety"] = {
                "score_0_100": humidity_safety,
                "raw_humidity_risk_0_100": float(humidity_risk),
            }
            weights_used["humidity"] = W_HUMIDITY

    # Calculate total score with explicit weight mapping
    if weights_used:
        ...
        for component_key, component_data in components.items():
            wk = weight_map.get(component_key)
            score_val = component_data.get("score_0_100")
            if wk in normalized_weights and score_val is not None:
                total_score += float(score_val) * normalized_weights[wk]
        ...
        total_stars = None
        normalized_weights = {}
        # return early if nothing to aggregate
        return {
